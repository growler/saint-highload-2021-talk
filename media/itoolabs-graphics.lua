...
function runcmd(params)
   ...
   if ret == nil or ret ~= 0 then
      if err ~= nil then
         ...
         debug('Error running command '..cmd..' (ret = '..ret..')')
      end
      return false
   end
   return true
...
local image_comparators = {
   -- PSNR
   function(src, dst) -- PSNR
      local stream = io.popen('magick compare -metric PSNR' .. ' "' .. src .. '" "' .. dst .. '" /dev/null 2>&1')
      if stream == nil then
         return nil
      end
      ...
      if string.match(out, "^inf$") then
         return "PSNR", 60, 60, 1
      elseif string.match(out, '^%d+%.%d+$') or string.match(out, "^%d$") then
         return "PSNR", tonumber(out), 60, 1
      else
...
function include_xkcd(name, docwidth, docheight, dpi)
   local stream = io.popen("curl -sL https://xkcd.com/"..name.."/ | xmllint --html --xpath 'string(//*[@id=\"comic\"]/img/@src)' - 2>/dev/null")
   local url = stream:read('*a')
   stream:close()
   local host, fname, ext = string.match(url, "^%s*//(.+)/([^/.]+).(.-)%s*$")
   debug("!!!", url, fname, ext)
   if fname == nil then
      error("failed to download xkcd://"..name)
   end
   local target = output_dir.."/xkcd-"..name.."."..ext
   if not runcmd({"curl", "-sLo", target, 'https://'..host..'/'..fname..'.'..ext}) then
      error("failed to download xkcd://"..name)
   end
   include_image(target, docwidth, docheight, true, dpi)
end
